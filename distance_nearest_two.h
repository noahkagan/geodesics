...
public:
  ...
  void load(const tinyobj::attrib_t &attrib,
  ...
  std::vector<float> propagate(int src) {
    ...
    std::vector<float> dist(adjacencies.size(),
    ...
    while (!queue.empty()) {
      ...
      for (auto iter = adjacencies[u].begin(); iter != adjacencies[u].end();
           ++iter) {
        size_t v = iter->first;
        std::cout << v << std::endl;
        float v_dist = u_dist + iter->second;
        for (auto iter2 = adjacencies[v].begin(); iter2 != adjacencies[v].end();
             ++iter2) {
          ...
          if (w != u) {
            ...
            if (w_dist < dist[w]) {
              dist[w] = w_dist;
              queue.push(std::make_pair(w_dist, w));
            }
          }
        }
        ...
        if (v_dist < dist[v]) {
          dist[v] = v_dist;
          queue.push(std::make_pair(v_dist, v));
        }
      }
    }
